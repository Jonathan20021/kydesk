...
class LandingController extends Controller
{
    public const FEATURES = [
        ...
        'quotes' => [
            ...
            'tagline' => 'Generador completo con PDF, ITBIS configurable, descuentos por línea y portal de aceptación · todo branded con tu logo',
            'icon' => 'file-text',
            'color' => '#7c5cff',
            'bg' => '#f3f0ff',
            'category' => 'NEGOCIO',
            'hero_kpis' => [['Plantillas', 'Reutilizables'], ['Impuestos', 'Configurables'], ['Aceptación', 'Online + Email']],
            'description' => 'Creá cotizaciones profesionales en segundos · agregá líneas de items con cantidad, precio, descuento por línea e impuesto individual · el sistema calcula subtotal, descuento global, ITBIS, envío y total en vivo · exportá un PDF con tu logo, color de marca y datos de pago, o mandalo al cliente con un link único donde puede revisar, descargar y aceptar online.',
            'steps' => [
                ['Configurá tu branding', 'Subí tu logo, definí colores, ITBIS o impuesto local, prefijo de numeración (COT-), validez por defecto y datos bancarios.', 'palette'],
                ['Armá la cotización', 'Cargá items manualmente, desde una plantilla o desde el catálogo de servicios. Totales se calculan en vivo · descuentos por línea o globales.', 'list-plus'],
                ['Enviala y trackeala', 'Mail al cliente con link único · ve cuándo la abre, la descarga y la acepta · notificación al equipo cuando dice "sí".', 'send'],
            ],
            'benefits' => [
                ['file-text','PDF profesional con mPDF','Header con tu logo en cada página, color brand, tabla zebra, totales destacados y línea de firma configurable.'],
                ['percent','ITBIS / IVA / VAT configurable','Seed con ITBIS 18% (RD), IVA 21% (ES/AR), IVA 16% (MX), VAT 7% y Exento · creá los que necesites con tasa decimal.'],
                ['receipt','Descuento por línea y global','Combinables · cálculo proporcional sobre la base imponible · items pueden marcarse exentos individualmente.'],
                ['link','Portal cliente con link único','Branding completo del tenant · descarga PDF · aceptación con firma digital (nombre + email) · rechazo con motivo.'],
                ['bookmark','Plantillas reutilizables','Items, intro, términos y validez prearmados · cargá una plantilla y la cotización queda lista en segundos.'],
                ['activity','Timeline + auto-tracking','Eventos created, sent, viewed, accepted, rejected, pdf_downloaded · auditoría de quién hizo qué y cuándo.'],
            ],
            'mockup' => 'quotes',
            'faqs' => [
                ['¿En qué planes está?','Business y Enterprise. El super admin puede activarlo por override en otros planes desde /admin/tenants/{id}/modules.'],
                ['¿Puedo personalizar el ITBIS o usar IVA?','Sí. Crea cuantos impuestos quieras desde Configuración → Impuestos · seteás cualquier % decimal · uno como default.'],
                ['¿El PDF lleva mi logo?','Sí. Subís el logo desde la configuración (JPG/PNG/WebP/SVG) y aparece en el header de cada página del PDF y en el portal público del cliente.'],
                ['¿Cómo sabe el cliente que le mandé una cotización?','Recibe un email con un CTA grande hacia el link único · ahí ve la cotización con tu branding · puede descargar el PDF y aceptarla online.'],
                ['¿Qué pasa cuando el cliente acepta?','El equipo recibe un email automático (configurable) · la cotización pasa a estado "aceptada" con timestamp + nombre + email del que firmó.'],
                ['¿Se conecta con el CRM y empresas?','Sí. Desde un lead en el CRM se prefijan los datos · al elegir una empresa registrada se autocompleta nombre/teléfono/dirección · items pueden cargarse desde el catálogo de servicios.'],
            ],
        ],
        'crm' => [
            'title' => 'CRM Leads & Clientes',
            'tagline' => 'Pipeline kanban, oportunidades, actividades y conversión a cliente · todo dentro del helpdesk',
            'icon' => 'contact-round',
            'color' => '#7c5cff',
            'bg' => '#f3f0ff',
            'category' => 'NEGOCIO',
            'hero_kpis' => [['Pipelines seed', '3'], ['Etapas pre-cargadas', '17'], ['Setup', '< 2 min']],
            'description' => 'Captá, gestioná y convertí leads sin salir del helpdesk. Pipeline visual con drag & drop, oportunidades con probabilidad heredada por etapa, lead scoring 0-100, rating Frío/Tibio/Caliente, actividades programadas (llamadas, emails, reuniones, WhatsApp), notas internas, tags multi-color y conversión a cliente con un click — creando empresa y usuario del Portal Cliente automáticamente.',
            'steps' => [
                ['Capturá el lead', 'Manual, formulario web, email o desde el chat. Score, rating y origen quedan registrados desde el día uno.', 'user-plus'],
                ['Movélo por el pipeline', 'Arrastrá la oportunidad entre etapas. La probabilidad se actualiza sola; las etapas Won/Lost cierran el deal.', 'kanban-square'],
                ['Convertilo en cliente', 'Un click crea la empresa en Companies y el acceso al Portal Cliente con contraseña temporal. Tickets se vinculan al instante.', 'user-check'],
            ],
            'benefits' => [
                ['kanban-square','Pipeline kanban con drag & drop','Múltiples pipelines (Ventas, Onboarding, Renovaciones) y etapas configurables con probabilidades calibradas.'],
                ['flame-kindling','Lead scoring + rating','Score 0-100 manual o calculado · Frío/Tibio/Caliente · top hot leads en el dashboard del CRM.'],
                ['phone-call','Actividades multi-canal','Llamadas, emails, reuniones, tareas, WhatsApp, SMS — con scheduled_at y alertas de follow-ups vencidos.'],
                ['radar','Orígenes y atribución','10 fuentes pre-cargadas (Web, Referido, Ads, LinkedIn, Cold call, Cold email, Evento, Partner, Form, WhatsApp).'],
                ['sticky-note','Notas internas + tags','Notas pinneables con autor y timestamp · 6 tags seed (Enterprise, SMB, VIP, Riesgo, Upsell, Hot lead).'],
                ['user-check','Conversión 360','Convertí lead a cliente · crea empresa, usuario del portal y vincula tickets existentes por email automáticamente.'],
            ],
            'mockup' => 'crm',
            'faqs' => [
                ['¿En qué planes está disponible?','Business y Enterprise. El super admin puede habilitarlo por override en otros planes desde /admin/tenants/{id}/modules.'],
                ['¿Puedo tener distintos pipelines para distintos servicios?','Sí. Creás los pipelines que necesites — Ventas, Onboarding, Renovaciones, Upsell — y cada lead puede tener oportunidades en varios.'],
                ['¿Cómo se relaciona con el módulo Companies?','Cuando convertís un lead a cliente, opcionalmente se crea la empresa automáticamente. Si ya existe, se vincula al lead. Los tickets entrantes con el email del lead se muestran en su ficha.'],
                ['¿Soporta importación masiva?','La carga manual y vía formulario web están listas. La importación CSV está en el roadmap inmediato.'],
                ['¿Hay permisos granulares?','Sí: crm.view, crm.create, crm.edit, crm.delete, crm.config, crm.assign, crm.convert. Owners y admins arrancan con todos; agentes con view+edit+create.'],
                ['¿Se integra con Reuniones e Igualas?','Sí. Las oportunidades pueden vincularse a una plantilla de iguala y a una reunión agendada — al ganarlas, queda todo encadenado.'],
            ],
        ],
        'retainers' => [
        ...
        'meetings' => [
            ...
            'tagline' => 'Página pública estilo Calendly + video conferencia integrada · Plan Business',
            'icon' => 'calendar-clock',
            'color' => '#7c5cff',
            'bg' => '#f3f0ff',
            'category' => 'OPERACIONES',
            'hero_kpis' => [['Conferencia', 'Jitsi + LiveKit'], ['Reservas', '24/7 sin fricción'], ['Disponible', 'Business']],
            'description' => 'Compartí un único enlace público y dejá que clientes reserven reuniones con tu equipo. Cada reserva auto-genera un room de video conferencia o llamada de audio embebido en el panel — host y cliente entran desde el mismo lugar, sin instalar nada. Análisis IA opcional al reservar (intent + sentiment + briefing pre-meeting). Disponible en Business y Enterprise.',
            'steps' => [
                ['Configurá tipos y disponibilidad','Definí tipos de reunión (demo, consulta, llamada técnica), duración, buffers, días bloqueados, preguntas custom.', 'settings-2'],
                ['Cliente reserva sin fricción','Página pública con calendario · 1 click para elegir horario · auto-genera room de Jitsi/LiveKit · email de confirmación con enlace directo.', 'mouse-pointer-click'],
                ['Conferencia en el panel','Click "Iniciar" en el detalle de la reunión · embed Jitsi/LiveKit dentro del workspace · recording opcional · sin alt-tab.', 'video'],
            ],
            'benefits' => [
                ['link-2','Enlace público único','Compartilo en email signatures, sitio web, WhatsApp · clientes reservan en el horario que les funcione.'],
                ['video','Video conferencia integrada','Auto-room por reunión · embebido o pestaña nueva · Jitsi gratis o 8x8.vc/self-host con JWT para producción.'],
                ['phone','Llamadas de audio','Cualquier tipo "phone" inicia con cámara apagada · ideal para llamadas técnicas rápidas.'],
                ['shield-check','LiveKit beta','Migración a LiveKit listo para activar · grabación, transcripción y SDK propio cuando quieras.'],
                ['sparkles','Kyros IA opcional','Detección automática de intent/sentiment/urgencia al reservar · briefing pre-meeting + email de follow-up.'],
                ['calendar-x','Control total','Bloqueá vacaciones, configura aviso mínimo, anticipación máxima, buffer antes/después · políticas por tipo.'],
            ],
            'mockup' => 'meetings',
            'faqs' => [
                ['¿Cómo funciona la video conferencia?','Auto-genera un room único por reunión usando Jitsi (gratis para empezar). Cuando el cliente y el host hacen click en "Entrar", caen en el mismo room — se ven y escuchan en tiempo real. Para producción seria con embed sin cortes, configurás 8x8.vc (Jitsi as a Service · tier gratis hasta 25 usuarios/mes) o self-host.'],
                ['¿Necesito instalar algo?','No. Todo corre en el navegador con WebRTC. Cliente entra desde el email de confirmación o desde la página de gestión, host desde el panel. Cero descargas.'],
                ['¿Y si quiero LiveKit en vez de Jitsi?','Soportamos LiveKit como provider alternativo · ya están firmados los access tokens y la migración es un cambio de setting. Activálo desde Ajustes cuando estés listo.'],
                ['¿La IA está incluida?','El módulo de meetings está en Business+. La IA aplicada a meetings (intent/sentiment/briefing/follow-up) requiere plan Enterprise + asignación de Kyros IA por el equipo Kydesk.'],
                ['¿Puedo cancelar/reprogramar yo y el cliente?','Sí. Cada reserva tiene una página de gestión pública con token único. Configurás por tipo si permitís cancelar/reprogramar desde el cliente.'],
            ],
        ],
        'ai_assist' => [
