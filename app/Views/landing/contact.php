    ...
    <div class="max-w-[1240px] mx-auto px-6 relative">
        <div class="max-w-3xl mx-auto text-center">
            <div class="inline-flex justify-center">
                <div class="aura-pill">
                    <span class="aura-pill-tag"><i class="lucide lucide-message-circle"></i> CONTACTO</span>
                    <span class="text-ink-700 font-medium">Te respondemos en menos de 24h</span>
                </div>
            </div>
            <h1 class="display-xl mt-8" style="text-wrap:balance;font-size:clamp(2.6rem,5vw + 1rem,5rem)">Hablemos de tu <span class="gradient-shift">soporte</span>.</h1>
            <p class="mt-7 text-[18px] text-ink-500 max-w-xl mx-auto leading-relaxed">Implementación, demos personalizadas, integraciones a medida. Estamos para hacer que tu equipo encuentre el helpdesk que merece.</p>
        </div>
    </div>
</section>
        ...
        <div class="lg:col-span-3 relative">
            ...
            <form action="mailto:jonathansandoval@kyrosrd.com" method="POST" enctype="text/plain" class="relative rounded-[28px] p-8 md:p-10" style="background:white;border:1px solid #ececef;box-shadow:0 30px 60px -20px rgba(124,92,255,0.18)">
                ...
                <div class="mt-7 space-y-4">
                    ...
                    <div>
                        <label class="label">¿De qué te interesa hablar?</label>
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            <?php foreach ([['Demo','play-circle'],['Ventas','tag'],['Integración','plug'],['Otro','circle-help']] as [$lbl,$ic]): ?>
                                <label class="cursor-pointer border border-[#ececef] rounded-xl p-3 text-center hover:border-brand-300 hover:bg-brand-50 transition has-[:checked]:border-brand-500 has-[:checked]:bg-brand-50 has-[:checked]:text-brand-700">
                                    <input type="radio" name="topic" value="<?= $lbl ?>" class="hidden">
                                    <i class="lucide lucide-<?= $ic ?> text-[16px] block mx-auto mb-1"></i>
                                    <span class="text-[11.5px] font-semibold"><?= $lbl ?></span>
                                </label>
                            <?php endforeach; ?>
                        </div>
                    </div>
